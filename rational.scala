...
class Rational(x: Int, y: Int) {
    ...
    def max(that: Rational): Rational = {
        if (this < that) that else this  // 'this' refers to current object
    }
...
// ============================================================================
// UTILITY FUNCTIONS OBJECT (Alternative Implementations)
// ============================================================================
object RationalUtils {
...
// ============================================================================
// MAIN OBJECT: Contains all executable code and comprehensive test suite
// ============================================================================
object RationalDemo {
    def main(args: Array[String]): Unit = {
        println("=== Rational Number Demonstration ===")
        
        // Run utility function tests first
        runUtilityTests()
        
        // Run comprehensive rational tests
        runRationalTests()
    ...
    /**
     * Test utility functions before testing Rational class
     */
    def runUtilityTests(): Unit = {
        println("\n--- Testing Utility Functions ---")
        
        // Test standalone GCD function
        assert(RationalUtils.gcd(12, 18) == 6, "GCD test failed")
        println("✓ GCD function works correctly")
    }
    
    /**
     * Comprehensive test suite for Rational class
     * Property-based and edge case testing
     */
    def runRationalTests(): Unit = {
        ...
        println("Chain subtraction result: " + (x - y - z))  // Chain subtraction: ((1/3 - 5/7) - 3/2)
        ...
        assert((x + y) * z == x * z + y * z, "Distributive law failed")  // Distributivity test
        println("✓ Distributive law verified")
        ...
        assert(r < sum, "Ordering test 1 failed")    // 1/4 < 1 should be true
        assert(s < sum, "Ordering test 2 failed")    // 3/4 < 1 should be true
        println("✓ Ordering operations work correctly")

        // COMMUTATIVITY TEST: max(a,b) should equal max(b,a)
        // Tests both comparison logic and max function
        assert(r.max(s) == (s.max(r)), "Commutativity of max failed")  // Test commutative property of max
        println("✓ Max function is commutative")
        ...
        assert(reducedSum.numer == sum.numer, "Reduction numerator failed")  // Numerator should be exactly 1
        assert(reducedSum.denom == sum.denom, "Reduction denominator failed")  // Denominator should be exactly 1
        println("✓ Fraction reduction works correctly")

        // NEGATION AND SIGN HANDLING TESTS
        // Verify negation preserves magnitude but flips sign
        println("Debug: -r.numer = " + (-r.numer))       // Should print: debug: -1  
        println("Debug: r negated = " + (-r).toString())  // Should print: DEBUG: r.neg: -1/4
        println("✓ Negation works correctly")
        ...
        assert((i + j) == k, "Auxiliary constructor test failed")        // Test: 5/1 + 7/1 = 12/1
        println("✓ Auxiliary constructor works correctly")
        ...
        println("Automatic reduction: " + r4)         // Should display "4/1" not "12/3"
        assert(r4.toString == "4/1", "Automatic reduction failed")
        println("✓ Automatic reduction verified")
    }
}